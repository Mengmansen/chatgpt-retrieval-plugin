...
function New-HardLinkOrCopy {
    ...
    $targetPath = Get-UniqueTargetPath -DestinationDirectory $DestinationDirectory -FileName $Source.Name
    $fallbackMessage = "Hardlinks sind nur innerhalb desselben Laufwerks möglich. Datei wird kopiert: $($Source.FullName)"

    try {
        $destinationResolved = (Resolve-Path -LiteralPath $DestinationDirectory -ErrorAction Stop).Path
    }
    catch {
        $destinationResolved = $DestinationDirectory
    }

    $sourceRoot = [System.IO.Path]::GetPathRoot($Source.FullName)
    $destinationRoot = [System.IO.Path]::GetPathRoot($destinationResolved)
